...
def output_image(display_name, save_name, image):
    print(save_name)
    new_name = save_name.split("/")
    cv2.imshow(display_name, image)
    cv2.imwrite(OUTPUT_DIRECTORY + new_name[len(new_name) - 1], image)
...
def run(image_name, currentTime):
    ...
    output_image('Mean & Median with balance %s filtered Image: %s' % (BALANCE_ALPHA, image_name),
                 '%s_%s.jpg' % (image_name, str(currentTime)), filtered_image)
